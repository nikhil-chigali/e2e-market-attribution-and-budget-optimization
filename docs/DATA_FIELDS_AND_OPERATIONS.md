# Criteo Attribution Dataset: Data Fields and Pipeline Operations

This document describes the data fields in the Criteo Attribution Modeling dataset and the operations and checks performed on the data in the pipeline. Use it as the reference for schema, transformations, and data quality checks. Field descriptions are based on the [Kaggle dataset page](https://www.kaggle.com/datasets/sharatsachin/criteo-attribution-modeling).
...
**How to run the pipeline:** Execute `python prefect_flows/criteo_etl.py` from the project root. Prerequisites: Kaggle credentials configured for [kagglehub](https://github.com/Kaggle/kagglehub) (e.g., `~/.kaggle/kaggle.json` or `KAGGLE_USERNAME`/`KAGGLE_KEY` environment variables).
...
| **cpo** | REAL | **Cost-per-order** (not revenue). **Only meaningful when attribution=1** (actual CPO); for other rows it is a placeholder/predicted value (bidding signal). *Disclaimer: transformed value, not the real price.* |
| **time_since_last_click** | INTEGER | Time since the last click (in seconds) for the given impression. **-1** if N/A. |
| **cat1** … **cat9** | TEXT | Contextual categorical features associated with the display. Meaning is undisclosed; used for click/conversion modeling (e.g., Hashing Trick in the original paper). |

---

## Pipeline Operations Performed

The following operations and checks have been implemented in the pipeline.
...
| **Bad attribution records** | Count where `attribution = 1` and `cpo = -1` | Monitor |
| **Timestamp range** | `min_timestamp`, `max_timestamp`, `timestamp_duration` (max − min) | Within campaign period |
| **Total rows** | `COUNT(*)` | — |
...
Creates a cleaned, analysis-ready table. Timestamp is treated as **relative only** (seconds from start); no calendar date.

| Step | Operation | Reasoning |
|------|-----------|-----------|
| **1. Timestamp (relative)** | Keep `timestamp` (INTEGER); add `day_number = (timestamp / 86400)::INT`, `hour_of_day = ((timestamp % 86400) / 3600)::INT` | Timestamp is not Unix epoch; relative seconds enable day/hour buckets without calendar interpretation. |
...
| **4. Sentinels** | `NULLIF(conversion_timestamp, -1)`, `NULLIF(conversion_id, '-1')`, `NULLIF(click_pos, -1)`, `NULLIF(click_nb, -1)`, `NULLIF(time_since_last_click, -1)` | -1 and '-1' become NULL; `click_pos` and `click_nb` are NULL when not a conversion. |
| **5. cpo split** | `cost_per_order_actual` = cpo only when attribution=1, else NULL; `cost_per_order_predicted` = cpo for all rows | cpo is only meaningful when attribution=1; predicted/placeholder value kept for analysis. |
| **6. time_period** | CASE on `hour_of_day` → 'morning', 'afternoon', 'evening', 'night' | Morning (6–11), afternoon (12–17), evening (18–22), night (else). |
| **7. Filter** | `WHERE cost > 0` | Drops invalid or zero-cost rows. |
| **8. Column selection** | Keep `impression_id`, `timestamp`, `day_number`, `hour_of_day`, `uid`, `campaign`, `click`, `conversion`, `conversion_timestamp`, `conversion_id`, **attribution**, `click_pos`, `click_nb`, `cost`, **cost_per_order_actual**, **cost_per_order_predicted**, `time_since_last_click`, `time_period`, `cat1`–`cat9` | Full click context and attribution (ground truth) for Phase 2. |

**Silver schema:** `impression_id`, `timestamp`, `day_number`, `hour_of_day`, `uid`, `campaign`, `click`, `conversion`, `conversion_timestamp`, `conversion_id`, `attribution`, `click_pos`, `click_nb`, `cost`, `cost_per_order_actual`, `cost_per_order_predicted`, `time_since_last_click`, `time_period`, `cat1`–`cat9`.
...
| Metrics | `total_impressions`, `total_clicks`, `total_cost`, `converted`, **criteo_attributed** (MAX(attribution)), **actual_cpo_sum** (SUM(cost_per_order_actual)), **avg_predicted_cpo** (AVG(cost_per_order_predicted)), **total_clicks_before_conversion** (MAX(click_nb)). |
...
| `actual_cpo_sum` | REAL | `SUM(cost_per_order_actual)` per user (only for attributed conversions). |
| `avg_predicted_cpo` | REAL | `AVG(cost_per_order_predicted)` per user (bidding signal). |
...
### 6. Gold Layer: campaign_touchpoints

**Task:** `create_campaign_touchpoints_table`  
**Location:** `src/prefect_tasks/gold.py`  
**Input:** `data/silver/clean_impressions.parquet`  
**Output:** `data/gold/campaign_touchpoints.parquet`

One row per impression enriched with user-journey context for attribution modeling.

| Operation | Description |
|-----------|-------------|
| Window context | `touchpoint_position` (ROW_NUMBER by uid, timestamp), `total_journey_length`, `user_first_timestamp`, `user_conversion_timestamp`, **user_attributed_by_criteo** (MAX(attribution) OVER uid), `previous_campaign` (LAG), **campaign_exposure_count** (ROW_NUMBER OVER uid, campaign ORDER BY timestamp). |
| Derived flags | `is_first_touch`, `is_last_touch`, `days_since_first_touch`, `days_until_conversion`, `position_normalized`, `is_repeat_campaign`, `cross_channel_flag`, **user_has_criteo_attribution**, **is_converting_click** (click=1 AND conversion=1), **is_first_click_in_sequence** (click_pos=0). |

**Gold campaign_touchpoints schema:** Silver columns `impression_id`, `uid`, `timestamp`, `day_number`, `hour_of_day`, `campaign`, `click`, `conversion`, `conversion_id`, `attribution`, `cost`, **cost_per_order_actual**, **cost_per_order_predicted**, `click_pos`, `click_nb`, `time_since_last_click`, `time_period`; plus `touchpoint_position`, `is_first_touch`, `is_last_touch`, `total_journey_length`, `days_since_first_touch`, `days_until_conversion`, `position_normalized`, `is_repeat_campaign`, `campaign_exposure_count`, `cross_channel_flag`, `user_has_criteo_attribution`, `is_converting_click`, `is_first_click_in_sequence`. Output uses **zstd** compression.
...
outputs/data_quality_report.json     data/silver/clean_impressions.parquet
                                                ↓  create_user_journeys_table    create_campaign_touchpoints_table
                                     data/gold/user_journeys.parquet    data/gold/campaign_touchpoints.parquet
```
...
| `src/prefect_tasks/gold.py` | Gold user_journeys and campaign_touchpoints |
